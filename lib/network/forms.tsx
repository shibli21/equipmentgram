  ...
  return useMutation(
    async (inspectionForm: InspectionForm): Promise<void> => {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (
          inspectionRequest.reportStatus === InspectionReportStatus.FilledForm
        ) {
...
export const useGetInspectionFormById = (id: string) => {
  return useQuery<InspectionForm, Error>(
...
export const useGetInspectionFormsByStatus = (status: string) => {
  return useQuery<InspectionFormWithId[], Error>(
    [InspectionFormsCollection, "inspection-forms", status],
    async () => {
      const q = query(
        collection(db, InspectionFormsCollection),
...
export const useUpdateInspectionFormStatus = (status: string) => {
  ...
  return useMutation(
    async (inspectionFormId: string): Promise<void> => {
      await runTransaction(db, async (transaction) => {
        const inspectionFormDocRef = doc(
          db,
          InspectionFormsCollection,
          inspectionFormId,
        );
        const inspectionFormDocSnap =
          await transaction.get(inspectionFormDocRef);
        ...
        const inspectionRequest =
          inspectionRequestDocSnap.data() as InspectionRequestObject;
        if (
